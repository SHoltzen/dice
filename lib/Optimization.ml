...
let rec upPass (e: CG.expr) : float list * tree =
  ...
  | Let(_, e1, e2) -> 
    ...
    (n1@n2, Branch(t1, t2))
  | And(e1, e2) | Or(e1, e2) | Eq(e1, e2) | Xor(e1, e2) | Tup(e1, e2) -> [], Leaf
  | Snd(e1) | Fst(e1) | Not(e1) | Observe(e1) -> [], Leaf
  | _ -> [], Leaf

  (* Replace the flips with corresponding variables *)
let rec downPass (e: CG.expr) 
  (fl: (String.t * float * (String.t * CG.expr) list) list) 
  (t: tree)
    : CG.expr * (String.t * float) list * (String.t * float * (String.t * CG.expr) list) list =
  ...
  and get_fl (vf: float list) (gl: (String.t * CG.expr) list)
              (c: (String.t * float * (String.t * CG.expr) list) list)
                : (String.t * float * (String.t * CG.expr) list) list =
    match vf with
    | [] -> c
    | head::tail -> 
      let var = fresh() in
      get_fl tail gl ((var, head, gl)::c)

  and update_fl (fl: (String.t * float * (String.t * CG.expr) list) list) 
  ...
  and merge_fl (fl1: (String.t * float * (String.t * CG.expr) list) list) 
    (fl2: (String.t * float * (String.t * CG.expr) list) list) 
    (c: (String.t * float * (String.t * CG.expr) list) list) 
    : (String.t * float * (String.t * CG.expr) list) list =
  ...
  and collapse_fl (fl: (String.t * float * (String.t * CG.expr) list) list) 
    (vl: String.t list)
    (vf: (String.t * CG.expr) list)
    (to_add: (String.t * CG.expr) list) 
    (pass_up: (String.t * float * (String.t * CG.expr) list) list)
    : (String.t * CG.expr) list * (String.t * float * (String.t * CG.expr) list) list =
  ...
  | Ite(g, thn, els) ->
    ...
    (match t with
      | Node((llist, rlist), left, right) -> 
        ...
        if new_flips = [] then
          if lower_flips = [] then
            ...
            (* Pass flips down *)
            let (thn_expr, thn_vf, thn_fl) = downPass thn fl2 left in
            let (els_expr, els_vf, els_fl) = downPass els fl2 right in
          ...
          (* Pass flips down *)
          let (thn_expr, thn_vf, thn_fl) = downPass thn pass_down_fl left in
          let (els_expr, els_vf, els_fl) = downPass els pass_down_fl right in
      ...
      | _ -> e, List.map (fun (v, f, gl) -> (v,f)) fl, fl)
  | Let(v, e1, e2) ->
    (match t with
    | Branch(t1, t2) -> 
      let (n1, vf1, fl1) = downPass e1 fl t1 in
      let (n2, vf2, fl2) = downPass e2 fl1 t2 in
      (Let(v, n1, n2), vf2, fl2)
    | _ -> e, List.map (fun (v, f, gl) -> (v,f)) fl, fl)
  | And(e1, e2)
  | Or(e1, e2) | Eq(e1, e2) | Xor(e1, e2) 
  | Tup(e1, e2) -> e, List.map (fun (v, f, gl) -> (v,f)) fl, fl
  | Snd(e1) | Fst(e1) | Not(e1) | Observe(e1) -> e, List.map (fun (v, f, gl) -> (v,f)) fl, fl
  | _ -> e, List.map (fun (v, f, gl) -> (v,f)) fl, fl
...
let flip_code_motion (e: CG.expr) (new_n: int) : CG.expr = 
  ...
  let (e1, vf1, fl1) = downPass e [] t in
  e1
