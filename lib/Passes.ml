...
let from_external_func_optimize mgr cfg sbk (tenv: EG.tenv) (f: EG.func) (flip_lifting: bool) (cross_table: bool) (branch_elimination: bool) (determinism: bool) : (EG.typ * CG.func) =
  (* add the arguments to the type environment *)
  let tenvwithargs = List.fold f.args ~init:tenv ~f:(fun acc (name, typ) ->
      Map.Poly.set acc ~key:name ~data:typ
    ) in
  let (t, conv) = from_external_expr mgr cfg true sbk tenvwithargs f.body in
  check_return_type f t;
  let optbody = Optimization.do_optimize conv !n flip_lifting cross_table branch_elimination determinism in
  (* convert arguments *)
  let args = List.map f.args ~f:(from_external_arg cfg) in
  (TFunc(List.map f.args ~f:snd, t), {name = f.name;
        args = args;
...
let from_external_prog_optimize ?(cfg: config = default_config) sbk (p: EG.program) (flip_lifting: bool) (cross_table: bool) (branch_elimination: bool) (determinism: bool) : (EG.typ * CG.program) =
  let mgr = Cudd.Man.make_d () in
  let (tenv, functions) = List.fold p.functions ~init:(Map.Poly.empty, []) ~f:(fun (tenv, flst) i ->
      let (t, conv) = from_external_func_optimize mgr cfg sbk tenv i flip_lifting cross_table branch_elimination determinism in
      let tenv' = Map.Poly.set tenv ~key:i.name ~data:t in
      (tenv', flst @ [conv])
    ) in
  let (t, convbody) = from_external_expr mgr cfg false sbk tenv p.body in
  let optbody = Optimization.do_optimize convbody !n flip_lifting cross_table branch_elimination determinism in
